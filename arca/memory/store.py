...
class MemoryStore:
    """Versioned SQLite memory with provenance and portable document search."""
        ...
        with self.connect() as db:
            db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS assertions (
                    ...
                    source TEXT NOT NULL, valid_from TEXT NOT NULL, superseded_at TEXT,
                ...
                CREATE TABLE IF NOT EXISTS documents (
                    ...
                    snippet TEXT NOT NULL, content TEXT NOT NULL, source TEXT NOT NULL,
                    fetched_at TEXT NOT NULL, UNIQUE(url)
                );
                CREATE TABLE IF NOT EXISTS episodes (
                    id TEXT PRIMARY KEY, created_at TEXT NOT NULL, objective TEXT NOT NULL,
                    task_kind TEXT NOT NULL, payload TEXT NOT NULL
                );
                CREATE TABLE IF NOT EXISTS bandit (
                    context TEXT NOT NULL, operator TEXT NOT NULL, trials INTEGER NOT NULL DEFAULT 0,
                    reward REAL NOT NULL DEFAULT 0, PRIMARY KEY(context, operator)
                );
            """)
    ...
    def remember(self, subject: str, predicate: str, object_: str, source: str = "user", confidence: float = 1.0) -> int:
        ...
        with self.connect() as db:
            cursor = db.execute("INSERT INTO assertions(subject,predicate,object,confidence,source,valid_from) VALUES(?,?,?,?,?,?)", (subject.casefold(), predicate.casefold(), object_, max(0.0, min(1.0, confidence)), source, utc_now()))
    ...
    def recall(self, query: str, limit: int = 8) -> list[dict[str, Any]]:
        ...
        params.append(limit)
        sql = f"SELECT subject,predicate,object,confidence,source,valid_from FROM assertions WHERE superseded_at IS NULL AND ({' OR '.join(clauses)}) ORDER BY confidence DESC, id DESC LIMIT ?"
        with self.connect() as db:
    ...
    def save_document(self, title: str, url: str, snippet: str, content: str, source: str = "web") -> None:
        with self.connect() as db:
            db.execute("""INSERT INTO documents(title,url,snippet,content,source,fetched_at) VALUES(?,?,?,?,?,?)
                ...
                content=excluded.content,source=excluded.source,fetched_at=excluded.fetched_at""", (title, url, snippet, content, source, utc_now()))
    ...
    def search_documents(self, query: str, limit: int = 5) -> list[dict[str, Any]]:
        ...
        clauses, params = [], []
        for token in tokens:
            clauses.append("(title LIKE ? OR snippet LIKE ? OR content LIKE ?)")
            value = f"%{token}%"
            params.extend([value, value, value])
        params.append(limit)
        with self.connect() as db:
    ...
    def save_episode(self, expediente: Expediente) -> None:
        with self.connect() as db:
            db.execute("INSERT OR REPLACE INTO episodes(id,created_at,objective,task_kind,payload) VALUES(?,?,?,?,?)", (expediente.id, expediente.created_at, expediente.objective, expediente.task_kind, json.dumps(expediente.to_dict(), ensure_ascii=False)))
    ...
    def reward(self, context: str, operator: str, value: float) -> None:
        with self.connect() as db:
            db.execute("""INSERT INTO bandit(context,operator,trials,reward) VALUES(?,?,1,?)
                ON CONFLICT(context,operator) DO UPDATE SET reward=(bandit.reward*bandit.trials+excluded.reward)/(bandit.trials+1), trials=bandit.trials+1""", (context, operator, value))
